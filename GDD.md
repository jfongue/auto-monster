...
> Version 0.66 — Document de référence du projet
...
>
> **v0.65 : fixes mise en page (combat/modals) + bandeau de zone en haut avec paysage** — retours utilisateur sur captures. Combat desktop : en-tête re-borné à la largeur de l'arène (480px, centré) et fenêtre plafonnée/centrée verticalement (fini le titre pleine largeur détaché + l'arène géante quasi vide) ; suppression de la règle `aspect-ratio:2.6/1` qui étirait l'arène et déséquilibrait les combattants (désormais symétriques 30/70). Modals : `.stat-summary` (Victoire) forcé en flex-bloc (la classe `chip-ico` le mettait en `inline-flex` → il partageait sa ligne avec « Continuer ») ; LevelUpDraft avec padding + options en `auto-fit` (remplissent la largeur) ; largeurs de modals bornées. Zone : bandeau en **haut de page** (bandeau d'équipe retiré du hub), bouton retour intégré, support d'une **image de fond de paysage** par zone (`Zone.bg`, ex. Vallée Sauvage) avec voile dégradé sombre + texte blanc pour la lisibilité (repli propre si l'image est absente). Vérifié en capture headless réelle (combat, modals, zone) + no-scroll toujours 0.
>
> **v0.66 : fix onboarding mobile (starter illisible/coupé au scroll)** — retour utilisateur : sur mobile, le premier pick de starter n'était pas visible à cause du scroll. Voir §0 et §0.bis.
>
...
### 2026-07-20 — v0.66
- [Onboarding/Mobile] Retour utilisateur : « on ne voit pas le premier pick à cause du scrolling ». Diagnostic : `.onboard` centrait `.ob-wizard` avec `align-items:center` sur son conteneur scrollable (`.game-shell > .onboard{overflow-y:auto}`) ; sur mobile, la `.starter-grid` passe en 1 colonne (`@media max-width:640px`) et devient plus haute que le viewport — bug classique flexbox : le débordement du **haut** d'un contenu centré via `align-items:center` est inatteignable au scroll (on ne peut scroller que vers le bas), donc le 1er starter (en haut de la grille) restait masqué et inaccessible. Fix (`game.css`) : `align-items/justify-content:center` retirés de `.onboard` (garde `display:flex`), remplacés par `margin:auto` sur `.ob-wizard` — centre toujours quand le contenu rentre, mais rend tout le débordement (haut et bas) scrollable normalement.
- **Vérification** : `tsc -b` propre, `vite build` OK, `engine.test.ts` 132/132 (aucun changement moteur, CSS seul).

### 2026-07-20 — v0.65
- [Combat/Layout] Retour utilisateur (captures) : « NME trop sur la gauche, pas de marge, stats placé bizarement ». Diagnostic via captures headless réelles : (1) l'en-tête de combat (`.combat-head`, titre + vitesses + Abandonner) s'étalait bord à bord alors que l'arène est plafonnée/centrée à ~460px → perçu « pas de marge » / désaligné ; (2) une règle `@media (min-width:900px){ .arena{ aspect-ratio:2.6/1 } }` (live-combat.css) étirait l'arène en large bandeau sur desktop, écartant/décalant les combattants (asymétrie perçue). Fix (`game.css` + `live-combat.css`) : `.combat-head` re-borné `max-width:480px; margin:0 auto` (aligné sur l'arène, désormais 480px) ; desktop `@media(min-width:900px)` : `.combat-fs-body{align-items:center}` + `.live-combat{flex:0 1 auto; height:min(100%,700px)}` (carte de combat compacte centrée verticalement au lieu d'une arène pleine hauteur) ; règle `aspect-ratio:2.6/1` supprimée. Combattants alignés sous leurs plaques. **Placement (itéré sur retour utilisateur « adversaire au milieu »)** : `space-evenly` puis `space-between` ne garantissaient pas la symétrie (d'autres éléments de l'arène perturbaient la répartition flex) → passage à un **positionnement absolu explicite** des `.fighter` (`position:absolute`, joueur `left:26%`, adversaire `left:74%`, `translateX(-50%)`), symétrique et bien étalé quelle que soit la largeur. Le moteur déplace le sprite (pas le `.fighter`) et mesure via `getBoundingClientRect` → intact. Aucun changement de logique moteur.
- [Modals] Même retour (« stats placé bizarement ») : (1) `RewardModal` — `.stat-summary` portait la classe `chip-ico` (`display:inline-flex`) et se retrouvait sur la MÊME ligne que le bouton « Continuer » ; forcé `display:flex; justify-content:center` (bloc) → empilement centré propre ; `.loot-box` passé en bloc centré (`width:fit-content; margin:auto`) ; `.modal.center` borné à 440px. (2) `LevelUpDraft` — le `.modal.wide` n'avait AUCUN padding (contenu collé en haut-gauche d'un modal large) et `.branch-choices` en grille 2 colonnes fixes laissait une option seule à gauche ; fix : classe `.modal.levelup` avec padding + largeur bornée, `.branch-choices` en `repeat(auto-fit,minmax(190px,1fr))` (les options remplissent la largeur), `role=dialog`/`aria-modal`.
- [Zone/Bandeau] Demande utilisateur : « la vallée sauvage doit avoir le bandeau en haut de la page, tu peux retirer le bandeau d'équipe ; ajoute ce paysage en image de fond (attention à la lisibilité) ». Fait : bandeau d'équipe (`.team-strip`) retiré du hub (forest+zone) → la vue de zone commence par son bandeau. `ZoneScreen` : bouton retour intégré dans le bandeau (coin haut-droit), et support d'une **image de fond** via nouveau champ `Zone.bg` (`data.ts`), appliqué à `vallee` (`/zones/vallee.jpg`). CSS `.zone-hero.has-bg` : `background-size:cover`, voile dégradé sombre (posé en inline-style AVANT l'`url()` pour la lisibilité) + textes en blanc/ombré, bouton retour translucide lisible ; **repli propre** si l'image est absente (le voile sombre sur la teinte de zone reste lisible). **Image fournie et intégrée** : l'utilisateur a déposé le paysage dans `maps/foret.png` ; compressé (ImageMagick, 1400px large, qualité 82 → ~230 Ko) et copié en `app/client/public/zones/vallee.jpg`. Rendu vérifié en capture : le paysage s'affiche sous le voile, « Vallée Sauvage » + barre restent lisibles en blanc.
- **Vérification** : captures headless réelles (Playwright) à 390px + 1280px de la zone Vallée, du combat, et des modals Victoire/level-up — tout conforme ; no-scroll toujours 0/0/0. `tsc -b` propre, `vite build` OK, `engine.test.ts` 132/132 (aucun changement moteur).
...
| Onboarding | Oui (§7) | ✅ **Wizard 4 étapes** (v0.23) : choix du monstre → lecture de fiche (**v0.61 : Rang/HP/Stamina + Trait de départ expliqués**, plus l'ancien Force/Armure/Vitesse) → **combat guidé** (**v0.42 : `LiveCombat tutorial`, combat interactif** vs Sprigling ; « tu pilotes ») → hub/boucle → adoption. **v0.66 :** fix scroll mobile (1er starter caché par un centrage flex piégeant le débordement du haut) |
